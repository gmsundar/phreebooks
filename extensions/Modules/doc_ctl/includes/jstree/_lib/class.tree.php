...
class _tree_struct {
	...
	protected $fields	= array(
			...
			"right"		=> false,
			"level"		=> false,
			"security"  => 'u:0;g:0', // ADDED BY PHREESOFT
		);

	// Constructor
	function __construct($table = TABLE_DC_DOCUMENT, $fields = array()) {
		$this->table = $table;
		if(!count($fields)) {
			...
			foreach($fields as $key => $field) {
				switch($key) {
					...
					case "right":
					case "level":
					case "security": // ADDED BY PHREESOFT
	...
	function _get_children($id, $recursive = false) {
		...
		while($this->db->nextr()) {
			...
			// check security
			if (!$row['security']) $row['security'] = 'u:0;g:0'; // allow all
			if (security_check($row['security'])) $children[$this->db->f($this->fields["id"])] = $row;
		}
		return $children;
	}
